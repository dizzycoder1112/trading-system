...
// Logger is the logging interface required by the gRPC server
...
//
// This allows the grpc package to be completely independent,
// while still supporting any logger implementation (zap, logrus, custom, etc.)
...
func (d *defaultLogger) log(level, msg string, context ...any) {
	...
	fmt.Printf("%s [%s] [grpc] %s", timestamp, level, msg)
